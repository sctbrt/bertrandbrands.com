...
- `/NCP` → Private client discovery questionnaire — standalone, tap-driven, sent directly to the client (`noindex, nofollow`, unlinked, not in sitemap; first build: Mike Fulcher / NCP Electrical)
...
| 13.6.2 | Apr 2026 | **Intro policy simplified to once-per-browser-session.** The v13.6.1 approach (device-wide `localStorage` flag + auto-dissolve cleanup + DOM removal) interacted badly with iOS Safari's Page Cache / bfcache, which could restore the homepage into a frozen mid-intro or end-frame state when the user tapped "Home" or swiped back from any sub-page. Switched the entire policy to **`sessionStorage` only**: intro plays exactly once per browser session (the very first homepage load in that tab), and every subsequent load in that session — fresh navigation, reload, bfcache restore, or anything else — skips straight to the hero via the existing `html.no-intro` critical-path CSS. `localStorage` is no longer read or written by the intro code. The synchronous `<head>` script now checks `sessionStorage.getItem('bb_intro_seen')` (plus `?skip` / `?intro` bypasses), and the IIFE sets the session flag at the moment the intro starts playing (not at the end). A `pageshow` listener with `event.persisted === true` still force-completes bfcache restores as a belt-and-suspenders. All other complexity (`localStorage` writes, referrer checks, stuck-intro heuristics, visibilitychange handler) removed — the code is ~50 lines shorter and has one clear mental model: new session → plays, same session → skipped. New sessions (new tab, new browser, restart) replay the intro naturally. Verified in preview: fresh session plays the full 4s + 0.8s + ~3s dissolve sequence; immediate navigation to `/intake` and back lands directly on the hero with no intro frame (200ms sample after return confirms `html.no-intro` + `intro-complete` + `heroAnimate` + intro element removed). |
| 13.6.1 | Apr 2026 | **Intro: true one-play-per-device + bfcache-safe.** Fixes the long-standing issue where swiping back to the homepage on mobile landed users on a frozen mid-intro frame (restored from iOS Safari's back/forward cache / Page Cache). Two-part fix in `src/pages/index.astro`: (1) `localStorage.setItem('bb_intro_seen', '1')` is now set **at the moment the intro starts playing**, not at the end — so if the user navigates away 1s into the intro, any subsequent fresh-load visit still treats the intro as seen and skips it. (2) Added a `pageshow` listener that fires `completeIntroImmediately()` when `event.persisted === true` (bfcache restore) — idempotent helper that removes the `.intro` element, adds `no-intro`/`intro-complete`/`intro-scrolled` classes, shows the header/hero, and sets the seen flag. Handler only fires on persisted restores, so fresh-load intro playback is untouched. Verified via synthetic `PageTransitionEvent` dispatch during mid-intro (intro element removed, completion classes applied) plus visual confirmation that the first-visit intro still plays end-to-end. |
| 13.6.0 | Apr 2026 | **Mobile scroll usability + wordmark refinement.** Root cause of "page locks at mid-lower position on mobile" was the homepage defaulting to scroll-snap on mobile (`body.scroll-snap-enabled` + `scroll-snap-type: y proximity` + `scroll-snap-align: start` on every `.hero` and `.section`). Every touch scroll snapped to a section boundary, making the page feel stuck. Default mobile scroll mode in `index.astro:1420` changed from `fullpage` → `free` (opt-in still available via `?scroll=fullpage`). (1) Homepage scroll-up snap-lock deleted (old lines 1269–1271) — users can scroll freely in both directions after intro dissolves. (2) Intro auto-cleanup: ~3.1s after fade triggers, `.intro` is removed from DOM, `body.intro-static` removed (dropping the 100vh `.page` margin), and scroll re-anchored so the user stays at the same visual position. Applied to both auto-dissolve path and manual-scroll safety-net path. (3) `100vh` → `100vh` + `100dvh` fallback on `.service-page` (ServiceDetailLayout, build/transform/care hubs) and `.intake-page`; `overflow: hidden` → `overflow-x: clip` on the same (preserves horizontal spotlight clipping without vertical-scroll interference on iOS). (4) Mobile menu resize failsafe — both `HeaderUniversal.astro` and homepage mobile menu release `body.style.overflow` if viewport widens past 1024px while menu is open (prevents orientation-change scroll lock). (5) Wordmark cleanup — `HeaderMinimal` simplified from multi-span `-short`/`-full`/`-sep`/`-sub` structure to single `<span>BERTRAND BRANDS</span>` matching `HeaderCanonical`/`HeaderUniversal`; obsolete CSS removed from `BaseLayout.astro`. All wordmarks gain `white-space: nowrap` + `@media (max-width: 360px) { display: none }` with icon-spacing adjustments — on iPhone SE 1st gen (320px) only the dotted B remains; "Bertrand Brands" never wraps. Touched 8 files. |
| 13.7.0 | May 2026 | **New intro: Pull Focus.** Replaces the dot-matrix scatter-and-converge that has been the default since v13.0 with a calmer photographic mechanic. The dotted B mark and the wordmark resolve together via a single shared focus-pull gesture: each B dot starts as a soft luminous blob (`filter: blur(10px) scale(1.4)`) and resolves to a crisp pixel; the wordmark animates the same way at type scale (`blur(40px) scale(1.04) letter-spacing(0.35em)` → crisp). Per-dot timing uses the existing random `--dot-delay` (0–0.4s) so dots resolve at slightly different paces — the "Polaroid developing unevenly" feel. Soft amber/violet/cool ambient haze sits at center throughout. Total runtime ~3.5s, then existing auto-dissolve + once-per-session policy unchanged. Variant infrastructure preserved: `?intro=a` replays the legacy scatter, `?intro=b` plays the synced flavour (all dots focus together), `?intro=d` plays the centre-out flavour (radial reveal from B centre). Files touched: `src/pages/index.astro` (default `'a'` → `'c'`), `public/styles/homepage.css` (replaced V14 variant block with shared `pullFocusDot` keyframe + per-variant delay strategies + mobile keyframe `pullFocusDotMobile` with lighter blur), `src/components/VersionStamp.astro`. Mobile uses `blur(6px) scale(1.3)` for GPU; haze backdrop drops from 120px → 60px blur. No JS letter-splitting needed (pure CSS now). |
| 13.8.0 | May 2026 | **Universal pill radius for all buttons + choice tiles.** New token `--radius-pill: 999px` added to `tokens.css` (alongside existing `--radius-sm/md/lg`). Single source of truth for fully-rounded radii. Applied to all primary CTAs and choice tiles: `.hero__cta` (homepage hero), `.offer-card__cta` (offer cards in 3 tier groups), `.tier-selector__option` (intake-prompt picker tiles), `.header__link--portal` (CLIENT PORTAL nav button), `.intake-simple__submit` + `.intake-nav__btn` (intake form submit + wizard navigation), `.intake-simple__field--radios > label` (simple-form radio choice tiles). The pre-existing `999px` literals in `shared.css` (back-link pill, footer CTA, gold-application pill) remain unchanged — they were already pill, just not yet tokenized; future cleanup pass can swap those to `var(--radius-pill)` for consistency. Cards (`.offer-card`, `.tier-card`, `.tier-wrapper`) stay rounded-rectangle (12–16px) — they're surfaces, not buttons. Files touched: `public/styles/tokens.css`, `public/styles/homepage.css`, `public/styles/shared.css`, `src/pages/intake.astro`, `src/components/VersionStamp.astro`. |
| 13.9.0 | May 2026 | **Removed Showcase nav dropdown (temporarily).** The "Showcase" header dropdown (B·POS / TheraKeep menu) is gone from the nav for now — the underlying anchor sections (`#bpos`, `#therakeep`) and product page `/bpos` are untouched, just no longer linked from the global header. Removed: dropdown markup in `HeaderCanonical.astro` + `HeaderUniversal.astro` (12 lines each), dropdown toggle JS in `HeaderUniversal.astro` (12 lines) and `index.astro` (12 lines), and the `:not(.header__dropdown-trigger)` exclusion in the mobile menu navLinks selector. Left intact: `.header__dropdown-*` CSS rules (inert without markup; can be reaped in a later cleanup pass), the BPOS + TheraKeep teaser cards on the homepage, and `/bpos` page. Files touched: `src/components/HeaderCanonical.astro`, `src/components/HeaderUniversal.astro`, `src/pages/index.astro`, `src/components/VersionStamp.astro`. |
| 13.10.0 | May 2026 | **V14 prelude — header declutter + magnetic-snap hamburger.** First incremental visible step toward the V14 visual foundation pass. (1) **Header trimmed to 2 items** on desktop: replaced 5 inline links + divider + Client Portal pill with a single `Menu` dropdown (Services / How It Works / About / Compare) plus `Get a Quote` CTA. The dropdown reuses the inert `.header__dropdown-*` CSS infrastructure left behind from v13.9.0's Showcase removal — no new CSS surface. (2) **Client Portal moved to footer** parent-line utility row (`FooterMain.astro` + `FooterMinimal.astro` + the inline footer on `index.astro`). Existing clients still find it; prospect-facing CTAs no longer compete with portal-link weight. (3) **Magnetic-snap toggle animation** replaces the 2014-era rotate-and-translate trick on `.header__toggle`. Three lines slide vertically to collide at center (`translateY(±11px)`), middle vanishes (`scale(0)` + opacity 0), outer two rotate ±45° to form X. Heavy under-/over-shoot bezier `cubic-bezier(0.9, -0.4, 0.1, 1.4)` over 0.46s gives the kinetic "snap" feel. Selected from a 15-variant lab at `/test-toggle` (noindex). (4) Restored `:not(.header__dropdown-trigger)` exclusion in mobile-menu navLinks selectors so the trigger doesn't auto-close the drawer; added dropdown toggle JS (IIFE) to both `HeaderUniversal.astro` and `index.astro`. Files touched: `HeaderCanonical.astro`, `HeaderUniversal.astro`, `FooterMain.astro`, `FooterMinimal.astro`, `index.astro` (mobile-menu selector + dropdown JS + inline footer), `BaseLayout.astro` (toggle CSS), `shared.css` (toggle CSS, kept in sync). Lab page `/test-toggle` added for the V14 toggle-animation A/B; noindex, unlinked. |
| 13.11.0 | May 2026 | **Private client discovery questionnaire shipped at `/NCP`.** New standalone, tap-driven discovery questionnaire for prospect / early-stage clients, served at the vanity URL `bertrandbrands.ca/NCP` (sent directly to the client; `noindex, nofollow`, unlinked, not in sitemap). First bespoke build is for Mike Fulcher / NCP Electrical. Self-contained Astro page (no `shared.css` deps) — a V14 surface using the locked tri-colour "gradient chrome only" palette (gradient on progress bar / primary buttons / hero accent word / ambient glow; amber retained on functional accents). On finish, answers POST to Formspree (`xnjjgoay`) + `/api/notify` (`source: 'discovery-ncp'`, label "NCP Discovery Session" — see §25.4), then a copy / download / print export is offered. Created by moving the form from its prior path under `src/pages/welcome/mike-fulcher/` to `src/pages/NCP.astro` (relative imports repointed; stale "no backend" header comments corrected). This deploy also carries the previously-uncommitted v13.10.0 header work (it had never been pushed). Files: `src/pages/NCP.astro` (new), `api/notify.ts`, `VersionStamp.astro`, `CLAUDE.md`. |
...
- `discovery-ncp` → "NCP Discovery Session" (v13.11+, fired by the standalone `/NCP` discovery questionnaire on finish)
